...
public class TumiCheak
{
    ...
    public bool Tumi = true;

    public TumiCheak(GameManager gm, string PlayerTurn)//選択した駒、現在の駒の配置状況
    {
        ...
        for (int k = 0;k < gm.komas2.Count;k++)
        {
            if (!Tumi)
            {
                break;
            }
            if (gm.komas2[k].GetComponent<komaManager>().Player != PlayerTurn)
            {
                this.X = gm.komas2[k].GetComponent<komaManager>().ListX;
                this.Y = gm.komas2[k].GetComponent<komaManager>().ListY;
                this.koma = gm.komas2[k].GetComponent<komaManager>();
                bool Pocket = gm.komas2[k].GetComponent<komaManager>().PocketFlg;
                string Player = gm.komas2[k].GetComponent<komaManager>().Player;
                this.komaName = gm.komas2[k].GetComponent<komaManager>().komaName;
    ...
    //private bool Tumi1 = false;//王が動ける
    //private bool Tumi2 = false;//ボード上にある駒であいこまができる
    //private bool Tumi3 = false;

    //private GameManager gm;
    //private GameObject go;//王手した駒
    //private string Player;

    //public TumiCheak(GameManager gm,int X,int Y,GameObject go)//相手の王の座標、及び王手した駒
    //{
    //    this.gm = gm;
    //    this.go = go;

    //    this.Player = PlayerContrlloer.PlayerTurn;
    //    for (int y = -1; y <= 1 && !Tumi1; y += 2)
    //    {
    //        for (int x = -1; x <= 1 && !Tumi1; x++)
    //        {
    //            Tumi1Check(x+ X, y + Y);
    //        }
    //    }
    //    for (int x = -1; x <= 1 && !Tumi1; x+=2)
    //    {
    //        Tumi1Check(x + X, Y);
    //    }
    ...
    //            if (gm.komas2[i].GetComponent<komaManager>().Player == Player)
    //            {
    //                if (gm.komas2[i].GetComponent<komaManager>().ListX != X || gm.komas2[i].GetComponent<komaManager>().ListY != Y)
    //                {
    //                    Tumi1 = true;
    //                    break;
    //                }
